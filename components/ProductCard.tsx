      ...
      <Link href={`/product/${product.handle}`} className={`flex flex-col ${compact ? "shrink-0" : "flex-1"}`}>
        <div className={`relative w-full shrink-0 bg-gray-100 ${compact ? "h-[185px]" : "aspect-[4/4]"}`}>
          {imageSrc && (
            <ProductImage
              ...
              alt={product.title}
              fill
              className={compact ? "object-cover" : "object-cover rounded-t-xl"}
              sizes={
                compact
                  ? "(max-width: 480px) 45vw, 180px"
                  : "(max-width: 480px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 280px"
              }
          ...
          {product.isNew && (
            <span className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded">Новинка</span>
          )}
        ...
        >
          ...
          {!compact && product.description && (
            <p className="text-sm text-gray-300 mb-2 line-clamp-3">{stripHtml(product.description)}</p>
          )}
          {!compact && product.rating && (
            <div className="flex items-center gap-1 mb-2 text-sm text-yellow-500">
              {"★".repeat(Math.round(product.rating))}
              <span className="ml-1 text-gray-500">({product.reviewsCount ?? 0})</span>
            </div>
          )}
